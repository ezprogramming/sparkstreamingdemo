...
object ClusterUber {
  ...
  def main(args: Array[String]) {
    ...
    val df: Dataset[Uber] = spark.read.option("inferSchema", "false").schema(schema).csv("/user/david/data/uber.csv").as[Uber]
    ...
    model.write.overwrite().save("/user/david/data/savemodel")
    val res = spark.sql("select dt, lat, lon, base, prediction as cid FROM uber order by dt")   
    res.write.format("json").save("/user/david/data/uber.json")
  }
}
